...
const ChatBotActions = ({ selectedPhoneNumber, selectedChat, isLoading }: ChatBotActionsProps) => {
  ...
  const pauseBot = async (phoneNumber: string, duration: number | null) => {
    try {
      const response = await fetch('https://n8n-c4ggs0sskw0kww00gco4888o.padariaidealsabores.com/webhook/pausa_bot', {
        method: 'POST',
        headers: {
  ...
  const startBot = async (phoneNumber: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const response = await fetch('https://n8n-c4ggs0sskw0kww00gco4888o.padariaidealsabores.com/webhook/inicia_bot', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
